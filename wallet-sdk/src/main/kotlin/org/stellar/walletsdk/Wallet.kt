...
class Wallet(
    ...
    init {
        require(
            applicationConfiguration.useHttp && stellarConfiguration.network != Network.PUBLIC
    ...
    var server: Server =
        if (horizonClient != null) {
            requireNotNull(submitClient) {
                "Horizon and submit client must be both initialized or " +
                        "set to null"
            }
...
internal val defaultBase64Decoder: Base64Decoder = {
    Base64
        .getDecoder()
        .decode(it)
}
